...
def transform_image(image, code_vectors):
    ...
    for pixel in image:
        ...
        for centroid in code_vectors:
            distances.append(np.linalg.norm(pixel - centroid, axis = 1)**2)
        distances = np.transpose(np.array(distances))
        result.append(code_vectors[np.argmin(distances, axis = 1)])
...
def kmeans_image_compression():
    im = plt.imread('baboon.tiff') # read an image from a file into an array; return value is a numpy.array; RGB image, return value is M*N*3
    N, M = im.shape[:2]
    im = im / 255
