...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
	...
	for i in range(1, max_iterations):
		g_n = np.mean(np.array(list(map(lambda x_n, y_n : (sigmoid(np.matmul(w_new.transpose(), np.array(x_n))) - y_n) * np.array(x_n), X_new, y))), axis = 0)
		w_new = w_new - step_size * g_n
...
def binary_predict(X, w, b):
	...
	TODO: add your code here
	"""      
	for x_n, i in zip(X, range(0, N)):
		p = sigmoid(np.matmul(w.transpose(), np.array(x_n)) + b)
		if p > 0.5:
			preds[i] = 1
		elif p < 0.5:
			preds[i] = 0
		else:
			preds[i] = np.random.choice(2, 1)[0]
...
def multinomial_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
	...
	X0 = np.ones((len(X), 1))
	X_new = np.hstack((X0, X))
	w = np.hstack((np.array([b]).transpose(), w))

	for i in range(0, max_iterations):
		# random choose a training sample X_new[n]
		# n = np.random.choice(N, 1)[0]
		for n in range(0, N):
			# convert y[n] to 1-to-K encoding y_nk
			y_nk = np.zeros(C)
			k = y[n]
			y_nk[k] = 1
			y_nk = y_nk.transpose()
			# compute gradients of error function w.r.t. parameters
			g_n = np.zeros((C, D+1))
			wT_xn = np.array(list(map(lambda w : np.matmul(np.array(w).transpose(), X_new[n]), w)))
			m = max(wT_xn)
			wT_xn = np.array(list(map(lambda w : np.exp(w - m), wT_xn)))

			s = sum(wT_xn)

			for k in range(0, C):
				g_n[k] = (wT_xn[k] / s - y_nk[k]) * X_new[n]
				w[k] = w[k] - step_size * g_n[k]

		# print(w)

	# for i in range(0, max_iterations):
	# 	# random choose a training sample x_n
	# 	n = np.random.choice(N, 1)[0]
	# 	# use every sample
	# 	# for n in range(0, N):
	# 	# convert y[n] to 1-to-K encoding y_nk
	...
	# 	g_n = np.zeros((C, D+1))
	# 	for c in range(0, C):
	# 		wT_x = list(map(lambda w_n : np.matmul(np.array(w_n).transpose(), X_new[n]), w))
	# 		max_wT_x = max(wT_x)
	# 		wT_x = np.array(list(map(lambda wtx : wtx - max_wT_x, wT_x)))
	# 		# print(wT_x)
	# 		num = np.exp(wT_x[c])
	# 		# num = np.exp(np.matmul(np.array(w[c]).transpose(), np.array(X_new[n])))
	# 		dom = sum(list(map(lambda w_n : np.exp(w_n), wT_x)))
	# 		# dom = sum(list(map(lambda w_n : np.exp(np.matmul(np.array(w_n).transpose(), X_new[n])), w)))
	# 		g_n[c] = (num / dom - y_nk[c]) * X_new[n]
	# 	w = w - step_size * g_n
...
def multinomial_predict(X, w, b):
	...
	for x_n, j in zip(X, range(0, N)):
		p = np.zeros(C)
		for i in range(0, C):
			p[i] = np.matmul(np.array(w[i]).transpose(), np.array(x_n)) + b[i]
		preds[j] = np.argmax(p)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
	...
	for i in range(0, C):
		...
		Y[i] = np.array(list(map(lambda yy : 1 if yy==i else 0, y)))
		# y_i = np.array(list(map(lambda yy : 1 if yy==i else 0, y)))
		# w[i], b[i] = binary_train(X, y_i)
		# print(w[i], b[i])
	# Y = Y.transpose()
	a = np.array(list(map(lambda yy : binary_train(X, yy), Y)))
	# print(a)
...
def OVR_predict(X, w, b):
	...
	for x_n, j in zip(X, range(0, N)):
		p = np.zeros(C)
		for i in range(0, C):
			p[i] = sigmoid(np.matmul(np.array(w[i]).transpose(), np.array(x_n)) + b[i])
		# print(p)
		preds[j] = np.argmax(p)
...
def run_multiclass():
	...
	datasets = [
	(toy_data_multiclass_3_classes_non_separable(), 'Synthetic data', 3)
						, 
