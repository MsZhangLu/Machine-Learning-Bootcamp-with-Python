...
def objective_function(X, y, w, lamb):
    ...
    for x_n, y_n in zip(X, y):
        ...
        m = max(0, 1 - y_n * np.matmul(np.transpose(w), x_n))
        # print(m)
        sum_max += m

    obj_value = lamb * w2 * 0.5 + sum_max / N
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        A_t_x_y = []
        for i in A_t:
            if ytrain[i] * np.matmul(np.transpose(w), Xtrain[i]) < 1:
                A_t_plus.append(i)
                A_t_x_y.append(ytrain[i] * Xtrain[i])
        
        A_t_x_y = np.array(A_t_x_y)
        yita = 1 / (lamb * iter)
        
        w_h = (1 - yita * lamb) * w + (yita / k) * np.transpose(np.array([np.sum(A_t_x_y, axis = 0)]))


        w = min(1, (1 / np.sqrt(lamb)) / np.sqrt(np.sum(w_h ** 2))) * w_h
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    sample_size = len(ytest)
    true_prediction = 0

    for x, y in zip(Xtest, ytest): 

        # print('product', np.matmul(np.transpose(w), x))
        predict = 1 if np.matmul(np.transpose(w), x) > t else -1
        # print('predict', predict)
        true_prediction += 1 if predict == y else 0

    # print('true_prediction', true_prediction)
    # print('sample_size', sample_size)
    test_acc = true_prediction / sample_size
    return test_acc
