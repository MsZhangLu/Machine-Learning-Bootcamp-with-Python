...
class linear_layer:
    ...
    def forward(self, X):
        ...
        # print(X.shape, len(self.params['b'][0]))
        forward_output = np.zeros((len(X), len(self.params['b'][0])))
        for i in range(0, len(X)):
            # print(i)
            forward_output[i] = np.matmul(X[i], self.params['W']) + self.params['b']            
    ...
    def backward(self, X, grad):
        ...
        # print(X.shape)
        # print(grad.shape, self.gradient['W'].shape)
        # print(grad.shape)
...
class relu:
    ...
    def forward(self, X):
        ...
        #         forward_output[i][j] = max(0, X[i][j])
        forward_output[X<0] = 0
        # print(forward_output[1][:10])
        return forward_output
